...
(() => {
  ...
  $$('[data-words]').forEach(el=>{if(!reduced.matches){el.querySelectorAll('br').forEach(br=>br.replaceWith(document.createTextNode(' ')));const text=el.textContent.trim();el.textContent='';text.split(/\s+/).forEach((word,i)=>{const outer=document.createElement('span'),inner=document.createElement('span');outer.className='w';inner.textContent=word+' ';inner.style.setProperty('--i',i);outer.append(inner);el.append(outer);});}observer.observe(el);});
  $$('[data-rise]').forEach(el=>{el.classList.add('rise');observer.observe(el);});
  $('[data-hero-stack]').classList.add('on');
  const tiles=['folder','code','design','image','duplicate','audio','video','memory','notes','process','archive','project'].map(n=>`assets/headroom/${n}.svg`);
